  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 p-4">
      <div className="flex justify-between items-center mb-8">
        ...
        {selectedCategory && (
          <Button color="primary" variant="light" onPress={() => setSelectedCategory(null)}>
            Back to Categories
          </Button>
        )}
      </div>
      ...
      {!selectedCategory ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {resourceCategories.map((category) => (
            <Card key={category.id} isPressable onPress={() => setSelectedCategory(category)} className="hover:scale-105 transition-transform bg-gradient-to-br from-primary-100 to-primary-200 dark:from-primary-700 dark:to-primary-800">
              <CardBody className="p-6">
                <h2 className="text-2xl font-semibold mb-2">{category.title}</h2>
                <p className="text-gray-600 dark:text-gray-300">{category.resources.length} resources available</p>
              </CardBody>
            </Card>
          ))}
        </div>
        ...
        <div className="space-y-4">
          ...
          {selectedCategory.resources.map((resource) => (
            <Card key={resource.id} className="bg-gradient-to-r from-secondary-100 to-secondary-200 dark:from-secondary-700 dark:to-secondary-800">
              <CardBody className="p-4">
                <div className="flex justify-between items-center">
                  <div>
                    <h3 className="text-xl font-semibold">{resource.title}</h3>
                    <p className="text-sm text-gray-500 capitalize">Type: {resource.type}</p>
                  </div>
                  <Button color="primary" variant="shadow" as="a" href={resource.link} target="_blank">
                    Access Resource
                  </Button>
                </div>
              </CardBody>
      ...
      <Card className="mt-8 bg-gradient-to-br from-tertiary-100 to-tertiary-200 dark:from-tertiary-700 dark:to-tertiary-800">
        <CardBody>
          <div className="flex justify-between items-center cursor-pointer" onClick={() => setIsScheduleExpanded(!isScheduleExpanded)}>
            <h2 className="text-3xl font-semibold">Your Daily Schedule</h2>
            <Button auto light onPress={() => setIsScheduleExpanded(!isScheduleExpanded)}>
            ...
            <div className="mt-4">
              ...
              {loading ? (
                ...
                <div className="space-y-4">
                  {schedule.length > 0 ? (
                    schedule.map((task) => (
                      <Card key={task._id} className="bg-white dark:bg-gray-800 shadow-md hover:shadow-lg transition-shadow">
                        <CardBody>
                          <div className="flex justify-between items-center">
                            <div>
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              <Button isDisabled color="primary" auto onPress={() => handleEditTask(task)}>
                                <EditIcon />
                              </Button>
                              <Button isDisabled color="error" auto onPress={() => handleDeleteTask(task._id)}>
                                <TrashIcon />
                              </Button>
                            </div>
                          </div>
                    ...
                    <p className="text-center text-gray-500">No tasks for the selected date</p>
                  )}
                </div>
              )}
            </div>
